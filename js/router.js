...
const ROUTES = {
  ...
  "#/viewer": { role: "center_lead", render: renderViewer },
};

function homeFor(profile) {
  if (!profile) return "#/login";
  if (profile.role === "admin") return "#/admin";
  if (profile.role === "center_lead") return "#/viewer";
...
export async function route() {
  ...
  if (profile.role === "viewer") {
    root().innerHTML =
      '<div class="card"><h2>접근 권한 없음</h2>' +
      "<p>이 시스템은 공동훈련센터 팀장·담당 계정만 열람할 수 있습니다.<br>" +
      "필요하시면 담당 강사에게 문의해 주세요.</p>" +
      '<button id="so">로그아웃</button></div>';
    document.getElementById("so").onclick = () => import("./auth.js").then(m => m.signOut());
    return;
  }
